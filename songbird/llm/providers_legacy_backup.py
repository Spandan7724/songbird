...
# LiteLLM provider creation functions
def create_litellm_provider(provider_name: str, model: str = None, api_base: str = None, **kwargs):
    ...
    from .litellm_adapter import create_litellm_provider as _create_litellm_provider
    return _create_litellm_provider(provider_name, model, api_base, **kwargs)


def get_litellm_provider(provider_name: str, model: str = None, api_base: str = None, **kwargs):
    """
    Get a LiteLLM provider instance with fallback to legacy providers.
    
    Args:
        provider_name: Provider name 
        model: Model name
        api_base: API base URL
        use_litellm: Whether to prefer LiteLLM
        **kwargs: Additional parameters
        
    Returns:
        Provider instance (LiteLLM or legacy)
    """
    try:
        ...
        console.print(f"[yellow]Falling back to legacy provider: {provider_name}[/yellow]")
        return get_legacy_provider(provider_name, model, **kwargs)


def get_legacy_provider(provider_name: str, model: str = None, **kwargs):
    """Get a legacy provider instance."""
    provider_class = get_provider(provider_name)
    
    # Initialize provider based on type and arguments
    if provider_name == "ollama":
        base_url = kwargs.get("base_url", "http://127.0.0.1:11434")
        return provider_class(base_url=base_url, model=model or "qwen2.5-coder:7b")
    else:
        # For all other providers, just pass the model
